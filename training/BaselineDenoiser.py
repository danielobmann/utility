...
from tensorflow.keras.layers import Input, Lambda, Conv2D, Conv2DTranspose, BatchNormalization, Activation
...
out = Conv2D(1, (3, 3), padding='same', activation='relu')(out)

# Make output operator consistent
out = Lambda(odl_op_layer_pseudo)(out)
out = Conv2D(64, (10, 10), padding='same')(out)
out = Conv2D(1, (1, 1), padding='same')(out)
out = Lambda(odl_op_layer)(out)

base_model = Model(inp, out)
base_model.summary()

inputs = tf.placeholder(shape=(None,) + inp_shape, dtype=tf.float32)
iterative_model = [base_model(inputs)]
iterative_model += [base_model(iterative_model[-1])]
iterative_model += [base_model(iterative_model[-1])]
iterative_model += [base_model(iterative_model[-1])]
iterative_model = tf.concat(iterative_model, axis=-1)

outputs = Conv2D(1, (1, 1), use_bias=False)(iterative_model)
y_true = tf.placeholder(shape=(None,) + inp_shape, dtype=tf.float32)


# ---------------------------
# Set up loss function for training
likelihood = tf.reduce_sum(tf.squared_difference(outputs, y_true))
...
def plot_validation(y_in, y_pred, y_true, epoch=10):
    ...
    for i in range(n_images):
        ...
        fig.savefig('images/BaselineValidationImage_Epoch' + str(epoch) + '_' + str(i) + '.pdf', format='pdf')
        fig.clf()
    pass
...
nmse = tf.reduce_mean(tf.reduce_sum(tf.square(y_true - outputs), axis=[1,2,3])/tf.reduce_sum(y_true**2, axis=[1,2,3]))
...
# ---------------------------
save_path = "models/baseline_denoising_network"
n_save = 20
n_val = 1
...
plt.savefig('images/baseline_loss.pdf', format='pdf')
